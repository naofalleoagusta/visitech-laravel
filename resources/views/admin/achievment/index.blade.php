...
@section('title', trans('admin.achievment.actions.index'))

@section('body')

    <achievment-listing
        :data="{{ $data->toJson() }}"
        :url="'{{ url('admin/achievments') }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.achievment.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                           href="{{ url('admin/achievments/create') }}" role="button"><i
                                class="fa fa-plus"></i>&nbsp; {{ trans('admin.achievment.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tr>
                                    ...
                                    <th is='sortable' :column="'id'">{{ trans('admin.achievment.columns.id') }}</th>
                                    <th is='sortable' :column="'name'">{{ trans('admin.achievment.columns.name') }}</th>
                                    <th is='sortable'
                                        :column="'enabled'">{{ trans('admin.achievment.columns.enabled') }}</th>
                                ...
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a
                                                    ...
                                                    @click="onBulkItemsClickedAll('/admin/achievments')"
                                        ...
                                        <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/achievments/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/achievments/create') }}"
                                   role="button"><i
                                        class="fa fa-plus"></i>&nbsp; {{ trans('admin.achievment.actions.create') }}</a>
                            </div>
                        </div>
